## Hi there 👋

<img 
  src="https://raw.githubusercontent.com/TommasoScalici/TommasoScalici/master/ts_logo.png" 
  alt="Tommaso Scalici Logo" 
  align="right"
  width="140" 
/>

I'm Tommaso Scalici, a **Software Developer | Artist | Playlist Curator** based in Palermo, Italy.

* 🚀 &nbsp; **Current Focus:** Building exceptional cross-platform mobile experiences with **Flutter** and exploring **AI-driven** solutions using the Google Cloud ecosystem (Vertex AI, Firebase).
* 🏗️ &nbsp; **Background:** Solid full-stack foundation in the **.NET/Azure** enterprise world (C#, WPF, SPFx), now evolving towards modern frontend and serverless architectures.
* 🎯 &nbsp; **Goal:** Combining clean code and architectural patterns to create intelligent, user-centric applications.

<br clear="all" />

### 🌎 Find me here

<a href="https://www.linkedin.com/in/tommasoscalici/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
<a href="https://tommasoscalici.altervista.org" target="_blank"><img src="https://img.shields.io/badge/Website-333333?style=for-the-badge&logo=Blogger&logoColor=white" alt="Sito Web"></a>

<br>
<br>

### 🏆 Certifications

* **Microsoft Certified:** Azure AI Fundamentals
* **Microsoft Certified:** Azure Fundamentals
* **Microsoft Certified:** Azure AI Engineer Associate
* **Microsoft Certified:** Azure Developer Associate
* **Google Cloud Skills Boost:** Prompt Design in Vertex AI

<br>

### 🔧 My Tech Stack

#### ☁️ Cloud & AI (Focus)
* **Google Cloud Platform:** Vertex AI, Firebase
* **Microsoft Azure:** Azure AI, Azure Functions, SharePoint (SPFx)
* **Concepts:** Prompt Engineering (RAG, CoT, ToT), Machine Learning Integration, Serverless Architecture

#### 💻 Core Languages
* **Dart** (Current Love)
* C#
* TypeScript / JavaScript
* SQL

#### 🚀 Frameworks & Ecosystems
* **Flutter** (Mobile & Cross-platform)
* **React**
* .NET / ASP.NET Core
* Blazor

#### 🛠️ Tools & Data
* **IDE:** VS Code, Visual Studio, Firebase Studio
* **DB:** SQL Server, MongoDB, Firestore
* **OS:** Linux & Windows
* **DevOps:** Git, Bash, PowerShell

<br>

### 📊 GitHub Stats

<a href="https://github.com/TommasoScalici">
  <img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=TommasoScalici&theme=dark&layout=compact">
</a>
<a href="https://github.com/TommasoScalici">
  <img align="center" src="https://github-readme-stats.vercel.app/api/?username=TommasoScalici&theme=dark&show_icons=true">
</a>

<br><br>

<a href="https://github.com/TommasoS/MathExtensions">
  <img align="center" src="https://github-readme-stats.vercel.app/api/pin/?username=TommasoScalici&repo=MathExtensions&theme=dark">
</a>
<a href="https://github.com/TommasoScalici/MVVMExtensions">
  <img align="center" src="https://github-readme-stats.vercel.app/api/pin/?username=TommasoScalici&repo=MVVMExtensions&theme=dark">
</a>

<br>
<br>

### 📈 Wakatime Stats

<figure>
  <img align="center" src="https://wakatime.com/share/@5eee5d0c-eeca-4a29-bdc6-46a11e610fb6/82dcedaf-97e5-4fcb-9ee4-03942fd11225.svg"></img>
</figure>
